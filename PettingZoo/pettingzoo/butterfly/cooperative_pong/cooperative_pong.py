...
class BallSprite(pygame.sprite.Sprite):
    ...
    def update2(self, area, p0, p1):
        ...
        side = self.get_side(area)
        coll_reward1 = 0
        coll_reward2 = 0
        if self.speed[0] != 0:
            done_x, coll_reward_1 = self.move_single_axis(self.speed[0], 0, area, p0, p1)
        if self.speed[1] != 0:
            done_y, coll_reward_2 = self.move_single_axis(0, self.speed[1], area, p0, p1)
        return (done_x or done_y), [side, (coll_reward_1 or coll_reward_2)]
    ...
    def move_single_axis(self, dx, dy, area, p0, p1):
        ...
        if not area.contains(self.rect):
            ...
            # right or left walls
                ...
                return True, reward_for_collision
        ...
        return False, reward_for_collision
...
class CooperativePong(gym.Env):
    def __init__(self, randomizer, ball_speed=9, left_paddle_speed=12, right_paddle_speed=12, cake_paddle=True, max_cycles=900, bounce_randomness=False):
        ...
        # paddles
        self.p0 = PaddleSprite((30 // RENDER_RATIO, 100 // RENDER_RATIO), left_paddle_speed)
        if cake_paddle:
            self.p1 = CakePaddle(right_paddle_speed)
        else:
            self.p1 = PaddleSprite((30 // RENDER_RATIO, 100 // RENDER_RATIO), right_paddle_speed)

        self.agents = ["paddle_0", "paddle_1"]  # list(range(self.num_agents))

        # ball
        self.ball = BallSprite(randomizer, (30 // RENDER_RATIO, 30 // RENDER_RATIO), ball_speed, bounce_randomness)
        self.randomizer = randomizer
    ...
    def step(self, action, agent):
        ...
        if agent == self.agents[0]:
            self.frame_continuation = random.randint(1,3)
            self.rewards = {a: 0 for a in self.agents}
            for i in range(0, self.frame_continuation):
            ...
            if not self.done:
                ...
                self.done, (agent_side, hit_paddle) = self.ball.update2(self.area, self.p0, self.p1)
                ...
                if self.done:
                    self.rewards[self.agents[agent_side]] = -1.0
                ...
                self.infos = {}
                for thing in self.agents:
                    self.dones[thing] = self.done
                    self.infos[thing] = False

                self.infos[self.agents[agent_side]] = hit_paddle
