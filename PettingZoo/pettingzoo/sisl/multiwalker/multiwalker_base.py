        ...
        for i, walker in enumerate(self.env.walkers):
            if walker.hull is not None:
                if walker.hull == contact.fixtureA.body:
                    if self.env.use_package:
                        if self.env.package != contact.fixtureB.body:
                            self.env.fallen_walkers[i] = True
                    else:
                        # either the walker has hit another walker or it has fallen, either way, terminate!
                        self.env.fallen_walkers[i] = True
...
class MultiWalkerEnv():
    ...
    def __init__(self, n_walkers=3, position_noise=1e-3, angle_noise=1e-3, local_ratio=1.0,
                 forward_reward=1.0, terminate_reward=-100.0, fall_reward=-10.0, terminate_on_fall=True,
                 remove_on_fall=True, max_cycles=500, use_package=False):
        """
        ...
        if not self.use_package:
            assert self.terminate_on_fall, 'If You arent using the package, then terminate on fall shld be true (otherwise walkers wont terminate)'
        self.frames = 0
    ...
    def reset(self):
        ...
        self.actions = {}
        r, d, o = self.scroll_subroutine(self.actions)
        self.last_rewards = [0 for _ in range(self.n_walkers)]
        self.last_dones = [False for _ in range(self.n_walkers)]
        self.last_obs = o
        self.frames = 0
    ...
    def scroll_subroutine(self, actions):
    ...
    def step(self, action, agent_id, is_last):
        ...
        if is_last:
            assert len(self.actions) == self.n_walkers, 'Error Here'

            self.world.Step(1.0 / FPS, 6 * 30, 2 * 30)
            rewards, done, mod_obs = self.scroll_subroutine(self.actions)
            self.actions = {}
