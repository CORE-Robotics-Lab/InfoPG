...
import ray
from distributed_pong_env import Pong_Worker, get_nagents_action_space
import numpy as np
import itertools
...
class Runner:
    def __init__(self, batch_size, num_workers = 4):
        ...
        self.envs = [Pong_Worker.remote(self.BATCH_SPLIT_SIZE, self.env_params, time_penalty=0.0, device='cpu') for _ in range(self.NUM_WORKERS)]
    ...
    def train_loop(self, save_path, transfer_experiment=None):
        ...
        model = Combined_Pong_Helper(device)
        ...
        for epoch in range(0, epochs):
            ...
            for agent_ix in range(self.N_AGENTS):
                ...
                epoch_returns[0, agent_ix, 0] = actor_agent.detach().item()
                epoch_returns[0, agent_ix, 1] = critic_agent.detach().item()
                epoch_returns[0, agent_ix, 2] = kernel_agent.detach().item()
    ...
    def get_agent_data(self, model: Combined_Pong_Helper, data, device):
...
if __name__ == '__main__':
    import argparse
    # python distributed_pong_train.py -batch 16 -workers 16
    # nohup python distributed_pong_train.py -batch 16 -workers 16 >> out.log &
